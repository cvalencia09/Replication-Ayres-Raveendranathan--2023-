...
integer :: iter_cal
double precision, parameter :: pow=.1
...
cut_off_data=1.4437959
    ...
    read(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota,cut_off_model 
    close(1)
...
!     abs(entrant_relative_size_data-entrant_relative_size))    
     ...
     theta1=theta*(firm_debt_gdp_data/(-firm_debt/ngdp))**(pow)
     psi1=psi*(h/emp_data)**pow
     alpha1=alpha*(labor_share_t(1)/lab_share_data)**pow
!      var_eps1=var_eps*(cv_i_k_data/(std_dev_i_k/avg_i_k))**pow
     var_eps1=var_eps*(std_dev_i_k_data/std_dev_i_k30check)**pow
!      var_eps1=var_eps*(std_dev_log_emp_19/std_dev_log_emp_by_age(20))**pow
!      var_iota=var_iota*(std_dev_log_emp_0/std_dev_log_emp_by_age(1))**pow
     k_e1=k_e!*(firms_5_firms0_5_data/(sum(firms_by_age(22:25))/sum(firms_by_age(2:25))))**pow
     f_o1=f_o*(entry_data/(entrants/(firms_by_age(2)+sum(omega_ss_initial))))**(pow/2)
     gamma=gamma*(i_k_inaction_data/i_k_inaction30)**pow
!     lambda=lambda*(i_k_pos_high_spike30/i_k_pos_high_spike_data)**pow
!      ex_mu=ex_mu*(entrant_relative_size_data/entrant_relative_size)**pow
     ex_mu=ex_mu*(firms_21_plus/firms_21_plus_data)**pow
     
         if ( rank .eq. 0 ) then
         
         print*,''
         print*,'iter_cal ',iter_cal,d_cal
         print*,'theta firm_debt/gdp'
         write(6,'(300F14.7)')theta,theta1,firm_debt_gdp_data,-firm_debt/ngdp
         ...
         print*,'var_eps std_dev_log_emp_19'
         write(6,'(300F14.7)')var_eps,var_eps1,std_dev_log_emp_19,std_dev_log_emp_by_age(20)           
         print*,'var_iota std_dev_log_emp_0'
         write(6,'(300F14.7)')var_iota,var_iota,std_dev_log_emp_0,std_dev_log_emp_by_age(1)           
         print*,'k_e firms_5_firms0_5'
         write(6,'(300F14.7)')k_e,k_e1,firms_5_firms0_5_data,(sum(firms_by_age(22:25))/sum(firms_by_age(2:25)))
         ...
         print*,'var_z exit_rate 26+'
         write(6,'(300F14.7)')var_z,var_z,.05,sum(exit_by_age(28:max_age))/sum(firms_by_age(28:max_age))
         print*,'mu entrant_relative_size'
...
!          open (unit=1,file='input_parameters.txt')
!          write(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota
!          close(1)
            end if
...
! psi_t(2)=1.07*psi_t(1)
!       
!     do t=3,90
!     psi_t(t)=.88*psi_t(t-1)+.12*psi_t(1)
!     end do
...
rho_shock=zero
      ...
      read(1,*)shock,rho_shock,psi_t(2),psi_c_t(2)
      close(1)
      
    do iter_shock=1,1

        do t=3,30
        psi_c_t(t)=rho_shock*psi_c_t(t-1)+(1-rho_shock)*psi_c_t(1)
        psi_t(t)=rho_shock*psi_t(t-1)+(1-rho_shock)*psi_t(1)
        end do
...
!     shock=shock*((minval(gdp_t(2:t1-1))/gdp_t(1))/gdp_drop_data1)**.75
!     rho_shock=rho_shock*((gdp_t(15)/gdp_t(1))/gdp_drop_data2)**.5
    
        if ( rank .eq. 0 ) then
        
        print*,'iter_shock',iter_shock
!         print*,'shock gdp_drop_data1 minval(gdp_t(2:t1-1))/gdp_t(1)'
!         write(6,'(300F12.4)')shock,gdp_drop_data1,minval(gdp_t(2:t1-1))/gdp_t(1)

!         print*,'rho_shock gdp_drop_data2 gdp_t(5)/gdp_t(1)'
!         write(6,'(300F12.4)')rho_shock,gdp_drop_data2,gdp_t(15)/gdp_t(1)

        print*,'psi_t(2) hours_drop_data hours_drop_model'
        write(6,'(300F12.4)')psi_t(2),0.852070167,h_t(2)/h_t(1)

        print*,'psi_c_t(2) c_drop_data hours_drop_model'
        write(6,'(300F12.4)')psi_c_t(2),0.898311195,c_t(2)/c_t(1)
        
!          open (unit=1,file='input_shock.txt')
!          write(1,*)shock,rho_shock,psi_t(2),psi_c_t(2)
!          close(1)

        end if

    psi_t(2)=psi_t(2)*((h_t(2)/h_t(1))/0.852070167)**.25
!     psi_c_t(2)=psi_c_t(2)*(0.898311195/(c_t(2)/c_t(1)))**.25
        ...
        call MPI_BCAST (psi_t(2),1,mpi_double_precision,0,mpi_comm_world,ierr)        
        call MPI_BCAST (psi_c_t(2),1,mpi_double_precision,0,mpi_comm_world,ierr)        
