...
double precision, dimension(t1) :: exit_lock_down_t, firm_lock_down_t
...
!                     end if
    ...
    !         h_t(t)=1-psi_t(t)*c_t(t)/w_t(t)
            ...
            iter_bi=0
                ...
                firm_lock_down_t(t)=zero
                exit_lock_down_t(t)=zero
                    ...
                    do ia=1,na
                        ...
                        kp=grid_k(jk)
                            ...
                            exits_t(t)=exits_t(t)+(1-d_o+d_o*delta_f)*(omega_ss0(ieps,ik,ia)+mu_t(t)*put_eps_t(t,ieps)*g_e_o_t(t,ieps)) 
                                ...
                                if ( ieps .gt. neta*nz ) then
                                firm_lock_down_t(t)=firm_lock_down_t(t)+(omega_ss0(ieps,ik,ia)+mu_t(t)*put_eps_t(t,ieps)*g_e_o_t(t,ieps))
                                exit_lock_down_t(t)=exit_lock_down_t(t)+(1-d_o)*(omega_ss0(ieps,ik,ia)+mu_t(t)*put_eps_t(t,ieps)*g_e_o_t(t,ieps))
                                end if
                            ...
                            exits_t(t)=exits_t(t)+(1-d_o+d_o*delta_f)*omega_ss0(ieps,ik,ia)    
                                ...
                                if ( ieps .gt. neta*nz ) then
                                firm_lock_down_t(t)=firm_lock_down_t(t)+omega_ss0(ieps,ik,ia)
                                exit_lock_down_t(t)=exit_lock_down_t(t)+(1-d_o)*omega_ss0(ieps,ik,ia)
                                end if
...
!             w_t=.7*w_t+(1-.7)*w_t1
            ...
            write(1,*)'t z psi theta tau_a f_e f_o tau_x tau_l C H B r w div entry mass rgdp rinv'
            write(1,*)'firms_constrained exit exit_b avg_firm_size labor_share std_dev_growth_emp'
            write(1,*)'std_dev_growth_sales debt capital ngdp ninv exdem psi_c collat_binding'
            write(1,*)'firm_lock_down exit_lock_down'
            write(1,*)
                do t=1,t1
                ...
                exdem_t(t),psi_c_t(t),collateral_binding_t(t),firm_lock_down_t(t),exit_lock_down_t(t)
                end do
            close(1)
