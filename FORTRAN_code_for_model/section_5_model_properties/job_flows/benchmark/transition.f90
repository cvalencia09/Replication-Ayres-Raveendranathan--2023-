...
iter_tr=0
    ...
    do while ( iter_tr .lt. itmax_tr .and. d_tr .ge. tol_tr )
            ...
            if ( rank .eq. 0 ) then
                ...
                g_e_o_t(t,:)=1
                    ...
                    open (unit=1,file='output_transition_entry_cutoff.txt',action='write',position='append')
                        do ieps=1,neps
                        write(1,'(300F12.7)')real(t),real(g_e_o_t(t,ieps)),v_i_recmpi(zmap_inv(ieps,intbirth_k,intbirth)),w_t(t)*f_e_t(t)+f_c+k_e,put_eps(ieps)
                        end do                    
                    close(1)            
