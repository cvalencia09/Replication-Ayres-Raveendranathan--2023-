...
!parameters
integer, parameter :: usespline=0, cap_adj=1        !0=no adjustment cost, 1=costly reversibility, 2=non-convex
integer, parameter :: ex_entry=1, default_economy=0, compute_job_flows=1
...
double precision :: z, abar, tau_a, psi, f_e, f_o, var_eps, tau_x, tau_l, k_e, theta, var_z
...
integer, parameter :: neps=neta*nz*niota, nk=250, na=100
...
double precision, parameter :: kmin=1e-13, kmax=400, amax=400, amin=-99
...
integer(kind=1), dimension(neps,nk,na) :: g_i_o, g_binding
...
integer(kind=1), dimension(neps,nk,na) :: g_i_o_initial
...
integer(kind=1), dimension(neps,nk,na) :: g_i_o_final
...
double precision, dimension(t1) :: c_t, h_t, b_t, exdem_t
double precision, dimension(t1) :: w_t, div_t, mu_t, r_t

double precision, dimension(t1) :: z_t, abar_t, tau_a_t, psi_t, f_e_t, var_eps_t, tau_x_t, tau_l_t, theta_t
...
double precision, dimension(t1,120) :: emp_by_age_t, firms_by_age_t, exit_by_age_t, exit_binding_by_age_t
...
double precision, dimension(max_age) :: avg_log_emp_by_age, std_dev_log_emp_by_age, dummy_by_age, debt_by_age, binding_by_age
...
double precision :: tot_cap_adjustment_cost, tot_binding
