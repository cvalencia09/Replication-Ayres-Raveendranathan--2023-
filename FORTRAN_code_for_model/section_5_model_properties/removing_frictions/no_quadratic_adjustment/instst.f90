...
!         end if
        ...
        do ia=1,na
            ...
            kp=grid_k(jk)
                ...
                tot_cap_adjustment_cost=tot_cap_adjustment_cost+func_adj(kp,k,d_o,date)*(omega_ss0(ieps,ik,ia)+mu*put_eps(ieps)*g_e_o(ieps))
                tot_binding=tot_binding+g_binding(ieps,ik,ia)*(omega_ss0(ieps,ik,ia)+mu*put_eps(ieps)*g_e_o(ieps))
...
!                         end if
                ...
                tot_cap_adjustment_cost=tot_cap_adjustment_cost+func_adj(kp,k,d_o,date)*omega_ss0(ieps,ik,ia)
                tot_binding=tot_binding+g_binding(ieps,ik,ia)*omega_ss0(ieps,ik,ia)                
...
!        print*,'iter_dist',iter_dist
    ...
    omega_ss1=zero
        ...
        do ia=1,na
            ...
            debt_by_age(iter_dist)=debt_by_age(iter_dist)-min(a,zero)*omega_ss0(ieps,ik,ia)
            binding_by_age(iter_dist)=binding_by_age(iter_dist)+g_binding(ieps,ik,ia)*omega_ss0(ieps,ik,ia)
...
!                 l=l+f_e                
                                ...
                                jk=g_i_k(ieps0,ik0,ia0)
                                    ...
                                    if ( g_i_o(ieps0,ik0,ia0) .eq. 1 ) then
                                        ...
                                        jz=id_z(jeps)
                                            ...
                                            if ( pi_eps(ieps0,jeps) .gt. zero ) then                        
                        
                                            ja=inds(ieps0,ik0,ia0,1)                            
                                            omega_ss3(jeps,jk,ja)=omega_ss3(jeps,jk,ja)+vals(ieps0,ik0,ia0,1)*pi_eps(ieps0,jeps)*omega_ss2(ieps0,ik0,ia0)*(1-delta_f)      

                                            ja=inds(ieps0,ik0,ia0,2)                        
                                            omega_ss3(jeps,jk,ja)=omega_ss3(jeps,jk,ja)+vals(ieps0,ik0,ia0,2)*pi_eps(ieps0,jeps)*omega_ss2(ieps0,ik0,ia0)*(1-delta_f)                                                    
    ...
    print*,'tot_cap_adjustment_cost/gdp',tot_cap_adjustment_cost/gdp
    print*,'tot_binding',tot_binding/(sum(omega_ss_initial)+mu*sum(put_eps(:)*g_e_o(:)))
    
        open (unit=1,file='output_life_cycle.txt')
        write(1,*)'age firms emp exit eps k a exit_bind job_creat job_destr div div_ope' 
        write(1,*)'avg_log_emp std_dev_log_emp debt binding'
            ...
            avg_log_emp_by_age(iter_dist),std_dev_log_emp_by_age(iter_dist),debt_by_age(iter_dist),binding_by_age(iter_dist)
            end do
        close(1)
    
    omega_ss0=omega_ss_initial
        
        open (unit=1,file='output_instst_size_all_firms.txt')
        write(1,*)'l firms'
        ...
        firms_dummy=zero
            ...
            do ia=1,na
                ...
                l=(w/(((Z*eps)**(1-alpha*nu))*(1-alpha)*nu*(k**(alpha*nu))))**(one/((1-alpha)*nu-1))+f_o*grid_z(iz)
                    ...
                    firms_dummy=firms_dummy+omega_ss0(ieps,ik,ia)
                
                        if ( ik .eq. intbirth_k .and. ia .eq. intbirth ) then
                        firms_dummy=firms_dummy+mu*put_eps(ieps)*g_e_o(ieps)
                        end if
            ...
            if ( firms_dummy .gt. zero ) then
            write(1,'(300F12.7)')l,firms_dummy
            end if
        ...
        !size distribution for entrants
        open (unit=1,file='output_instst_size_entrants.txt')
        write(1,*)'l entrants'
        ...
        omega_ss1=zero
            ...
            do ia=1,na
                ...
                l=(w/(((Z*eps)**(1-alpha*nu))*(1-alpha)*nu*(k**(alpha*nu))))**(one/((1-alpha)*nu-1))+f_o*grid_z(iz)
                    ...
                    write(1,*)l,omega_ss0(ieps,ik,ia)
    ...
    !size distribution for exits
    omega_ss0=omega_ss_initial
    
        do ieps=1,neps
        omega_ss0(ieps,intbirth_k,intbirth)=omega_ss0(ieps,intbirth_k,intbirth)+mu*put_eps(ieps)*g_e_o(ieps)
        end do
        
        open (unit=1,file='output_instst_size_exit.txt')
        write(1,*)'l exits'
            ...
            omega_ss2=zero                
                ...
                do ia=1,na
                    ...
                    if ( omega_ss0(ieps,ik,ia) .gt. zero .and. g_i_o(ieps,ik,ia) .eq. 1 )  then
                    ...
                    omega_ss2(ieps,ik,ia)=omega_ss2(ieps,ik,ia)+omega_ss0(ieps,ik,ia)
        
                    end if
                
                end do
                
            firms_dummy=sum(omega_ss2)
                ...
                omega_ss3=zero                    
                    ...
                    d_o=g_i_o(ieps0,ik0,ia0)
                    jk=g_i_k(ieps0,ik0,ia0)
                    ap=g_i_a(ieps0,ik0,ia0)        
                    kp=grid_k(jk)
                        ...
                        if ( d_o .eq. 1 ) then
                            ...
                            do jeps=1,neps
                                ...
                                omega_ss3(jeps,jk,inds(ieps0,ik0,ia0,1))=omega_ss3(jeps,jk,inds(ieps0,ik0,ia0,1))+&
                                vals(ieps0,ik0,ia0,1)*pi_eps(ieps0,jeps)*omega_ss2(ieps0,ik0,ia0)*(1-delta_f)
                                omega_ss3(jeps,jk,inds(ieps0,ik0,ia0,2))=omega_ss3(jeps,jk,inds(ieps0,ik0,ia0,2))+&
                                vals(ieps0,ik0,ia0,2)*pi_eps(ieps0,jeps)*omega_ss2(ieps0,ik0,ia0)*(1-delta_f)
                ...
                if ( firms_dummy-sum(omega_ss3) .gt. zero ) then
                write(1,'(300F12.7)')l,firms_dummy-sum(omega_ss3)
                end if
...
!            x_i=kp-(1-delta_k)*k*d_o-chi*k*(1-d_o)
            ...
            temp=temp+omega_temp0(ik0,ieps,ik,ia)
                ...
                if ( abs(x_i/k) .le. 3*std_dev_i_k_data ) then
...
!                        close(1)                            
    ...
    print*,'avg_i_k30',avg_i_k30
    print*,'std_dev_i_k30',std_dev_i_k30,temp_k
    ...
    print*,'std_dev_i_k30check',std_dev_i_k30check,temp
