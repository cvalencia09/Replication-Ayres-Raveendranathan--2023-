            ...
            if ( ja .lt. intbirth ) then
                ...
                g_q(ieps,jk,ja)=zero                
                    ...
                    do jeps=1,neps
                        ...
                        (g_i_o_recmpi(zmap_inv(jeps,jk,ja))*(-grid_a(date,ja))+&
                        (1-g_i_o_recmpi(zmap_inv(jeps,jk,ja)))*min(-grid_a(date,ja),max(zero,(1.+tau_x)*grid_k(jk)*chi&
                        -func_adj(zero,grid_k(jk),g_i_o_recmpi(zmap_inv(jeps,jk,ja)),date))))
                        end if
...
!             call exit
        ...
        if ( date .eq. 1 ) then
            ...
            write(1,*)'ap q'
                do ja=1,na
                write(1,'(300F12.7)')grid_a(date,ja),(g_q(ieps,intbirth_k,ja),ieps=1,neps)
                end do
            close(1)
