...
integer :: iter_cal
double precision, parameter :: pow=.2
...
cut_off_data=1.4437959
    ...
    read(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota,cut_off_model,f_o_good
    close(1)
    
!0.3592106,0.7676643,0.4110570,0.0061901,0,0.0121018,3.1528608,0.1888045,0.300000011920929,1.63540537081020,1.5438118
f_o=zero
...
!      var_iota=var_iota*(std_dev_log_emp_0/std_dev_log_emp_by_age(1))**pow
     ...
     f_o_good=f_o_good**(entry_data/(entrants/(firms_by_age(2)+sum(omega_ss_initial))))**(pow/2)
...
!         write(6,'(300F14.7)')var_iota,var_iota,std_dev_log_emp_0,std_dev_log_emp_by_age(1)           
         ...
         print*,'f_o_good entry_rate'
         write(6,'(300F14.7)')f_o_good,f_o_good,entry_data,entrants/(firms_by_age(2)+sum(omega_ss_initial))
...
!                open (unit=1,file='input_parameters.txt')
!                write(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota,cut_off_model,f_o_good
!                close(1)
            end if
...
rho_shock=.88
    ...
    shock=shock*(gdp_drop_data1/(minval(gdp_t(2:t1-1))/gdp_t(1)))**2.75
    rho_shock=rho_shock*((gdp_t(15)/gdp_t(1))/gdp_drop_data2)**2.75
