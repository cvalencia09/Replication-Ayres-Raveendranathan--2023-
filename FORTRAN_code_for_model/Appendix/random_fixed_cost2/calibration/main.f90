...
cut_off_data=1.4437959
    ...
    read(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota,cut_off_model,prob_death
    close(1)
...
!     prob_death=prob_death*(entry_data/(entrants/(firms_by_age(2)+sum(omega_ss_initial))))**(pow)
     f_o1=f_o*(entry_data/(entrants/(firms_by_age(2)+sum(omega_ss_initial))))**(pow)
     gamma=gamma*(i_k_inaction_data/i_k_inaction30)**(pow/2)
     lambda=lambda*(i_k_pos_high_spike30/i_k_pos_high_spike_data)**(pow/2)
...
!         open (unit=1,file='input_parameters.txt')
!         write(1,*)theta,psi,alpha,var_eps,f_o,k_e,gamma,lambda,ex_mu,var_iota,cut_off_model,prob_death
!         close(1)
            end if
