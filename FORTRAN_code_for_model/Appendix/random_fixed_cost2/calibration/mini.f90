...
integer(kind=1), allocatable, dimension(:) :: g_i_o_mpi, g_binding_mpi
...
integer(kind=1), allocatable, dimension(:) :: g_i_o_recmpi, g_binding_recmpi
...
integer :: ja_temp
    ...
    allocate (g_i_o_recmpi(np*nproc),g_i_k_recmpi(np*nproc),g_i_a_recmpi(np*nproc),v_i_recmpi(np*nproc),e_v_i_recmpi(np*nproc),g_binding_recmpi(np*nproc))
...
!initial guess for value function
    ...
    read_v_i=0
        ...
        (1.+tau_x)*(1-delta_k)*k+a-func_adj(kp,k,d_o,date)
...
iter_v=0
    ...
    !bond price schedule given operate/exit decisions    
        ...
        !incumbent's problem
        allocate (g_i_o_mpi(np),g_i_k_mpi(np),g_i_a_mpi(np),v_i_mpi(np),g_binding_mpi(np))
        ...
        kp=grid_k(jk)
            ...
            ap=a_un(ieps,ik)             
            profit=((Z*eps)**(1-alpha*nu))*(((k**alpha)*(l**(1-alpha)))**nu)-w*(l+f_o*grid_z(iz))+&
            (1.+tau_x)*((1-delta_k)*k-kp)+a-ap/(1+r)-func_adj(kp,k,d_o,date)
...
!                 if ( profit .ge. zero .or. profit .lt. zero ) then
                ...
                kpmax=kmax
                    ...
                    (1.+tau_x)*((1-delta_k)*k-kp)+a-func_adj(kp,k,d_o,date) 
                        ...
                        ap=constant/q
                            ...
                            print*,'fix default, vfn_i, mini.f90'
                                call exit                        
                            end if
        ...
        call mpi_allgather (g_binding_mpi,np,mpi_integer1,g_binding_recmpi,np,mpi_integer1,mpi_comm_world,ierr)    

        deallocate (g_i_o_mpi,g_i_k_mpi,g_i_a_mpi,v_i_mpi,g_binding_mpi)
...
!         end if
    ...
    g_binding(ieps,ik,ia)=g_binding_recmpi(zmap_inv(ieps,ik,ia))
    ...
    deallocate (g_i_o_recmpi,g_i_k_recmpi,g_i_a_recmpi,v_i_recmpi,e_v_i_recmpi,g_binding_recmpi)
...
!initial guess for value function
    ...
    read_v_un=0
        ...
        l=(w/(((Z*eps)**(1-alpha*nu))*(1-alpha)*nu*(k**(alpha*nu))))**(one/((1-alpha)*nu-1))
        profit=((Z*eps)**(1-alpha*nu))*(((k**alpha)*(l**(1-alpha)))**nu)-w*(l+f_o*grid_z(iz))+&
        (1.+tau_x)*(1-delta_k)*k-func_adj(kp,k,d_o,date)
    ...
    do while ( iter_v .le. 1500 .and. d_v .ge. tol_v )
        ...
        jk_temp=-1
            ...
            kp=grid_k(jk)
                ...
                profit=((Z*eps)**(1-alpha*nu))*(((k**alpha)*(l**(1-alpha)))**nu)-w*(l+f_o*grid_z(iz))+&
                (1.+tau_x)*((1-delta_k)*k-kp)-func_adj(kp,k,d_o,date)                        
            ...
            if ( iz .eq. 2 .and. g_un_o_mpi(cnt-itop_un+1) .eq. 1 ) then
            print*,'iz .eq. 2 .and. g_un_o_mpi(cnt-itop_un+1) .eq. 1, vfn_un,mini.f90'
                call exit
            end if
    ...
    if ( rank .eq. 0 ) then
        ...
        open(unit=1,file='output_profit_unconstrained.txt')
            ...
            write(1,'(300F12.7)')grid_k(ik),(((Z*grid_eps(ieps))**(1-alpha*nu))*(((grid_k(ik)**alpha)*&
                ...
                (1.+tau_x)*((1-delta_k)*grid_k(ik)-grid_k(g_un_k(ieps,ik))),ieps=5,neps,5)
            end do
        close(1)
...
!            call exit
    ...
    if ( maxval(a_un) .gt. amax .and. rank .eq. 0 ) then
    print*,'increase amax, vfn_un, mini.f90',maxval(a_un),amax
        call exit
    end if
    
    if ( rank .eq. 0 ) then
    print*,'maxval(a_un)',maxval(a_un)
    end if
