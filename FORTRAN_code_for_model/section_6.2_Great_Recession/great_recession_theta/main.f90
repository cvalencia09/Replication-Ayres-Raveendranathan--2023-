...
integer :: t
double precision, dimension(t1) :: z_t_data
...
! experiment
    open (unit=1,file='input_z_t_data.txt')
        do t=2,41
        read(1,*)z_t_data(t)
        end do
    close(1)
...
shock=.6*theta_t(1)
rho_shock=.95
z_t(2:41)=z_t_data(2:41)

    open (unit=1,file='input_shock.txt')
    read(1,*)shock,rho_shock!,z_t(2:41)
    close(1)
...
z_t=one
    ...
    theta_t(2)=shock
        ...
        do t=3,90
        theta_t(t)=rho_shock*theta_t(t-1)+(1-rho_shock)*theta_t(1)
        end do
        ...
        dummy=minval(-firm_debt_t(1:20))/(-firm_debt_t(1))                    
        
        print*,'iter_shock',iter_shock
        
        print*,'shock firm_debt_gdp_peak_to_trough'
        write(6,'(300F12.4)')shock,0.83998457,dummy

        print*,'rho_shock firm_debt_gdp_15th_quarter'
        write(6,'(300F12.4)')rho_shock,0.858374709,-firm_debt_t(15)/(-firm_debt_t(1))

!         print*,'z_t labor productivity'    
!         
!             do t=2,41
!             write(6,'(300F12.4)')real(t),z_t(t),z_t_data(t),(gdp_t(t)/h_t(t))/(gdp_t(1)/h_t(1))
!             end do
                        
        shock=shock*(0.83998457/dummy)**.15
        rho_shock=rho_shock*((-firm_debt_t(15)/(-firm_debt_t(1)))/0.858374709)**.15
        rho_shock=min(rho_shock,.99)


!             do t=2,41
!             z_t(t)=z_t(t)*(z_t_data(t)/((gdp_t(t)/h_t(t))/(gdp_t(1)/h_t(1))))**.5
!             end do
    
!         open (unit=1,file='input_shock.txt')
!         write(1,*)shock,rho_shock!,z_t(2:41)
!         close(1)
...
!             do t=2,41
!             call MPI_BCAST (z_t(t),1,mpi_double_precision,0,mpi_comm_world,ierr)        
!             end do
