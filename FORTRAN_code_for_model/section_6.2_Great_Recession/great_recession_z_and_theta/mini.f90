...
integer :: ja_temp
    ...
    g_binding_collateral_recmpi(np*nproc))
...
iter_v=0
    ...
    do while ( iter_v .le. itmax_v .and. d_v .ge. tol_v )
        ...
        !incumbent's problem
        allocate (g_i_o_mpi(np),g_i_k_mpi(np),g_i_a_mpi(np),v_i_mpi(np),g_binding_collateral_mpi(np))
...
!                 if ( profit .ge. zero .or. profit .lt. zero ) then
        ...
        deallocate (g_i_o_mpi,g_i_k_mpi,g_i_a_mpi,v_i_mpi,g_binding_collateral_mpi)
...
!             call exit
    ...
    deallocate (g_i_o_recmpi,g_i_k_recmpi,g_i_a_recmpi,v_i_recmpi,e_v_i_recmpi,g_binding_collateral_recmpi)
